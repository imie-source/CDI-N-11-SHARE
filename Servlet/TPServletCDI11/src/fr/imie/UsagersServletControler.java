...
public class UsagersServletControler extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		RequestDispatcher requestDispatcher = request
				.getRequestDispatcher("/UsagersServletView");
	...
	protected void doPost(HttpServletRequest request,
		...
		if (request.getParameter("delete") != null) {
			...
			UsagerDTO dto = usagers.get(numLigneInteger - 1);
			//UsagerDAO usagerDAO = new UsagerDAO();
			response.sendRedirect(String.format("UsagerServletFormControler?numLigne=%d",numLigneInteger));
			//usagerDAO.delete(dto);
			// 1ere solution : modifier la session et afficher la vue
			// request.getRequestDispatcher("/UsagersServletView").forward(request,
			// response);
			...
			response.sendRedirect("UsagerServletFormControler");
		}
	}
